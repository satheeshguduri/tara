...
class ShopCategoryDetailsScreen extends StatefulWidget {
  ...
  ShopCategoryDetailsScreen({Key key, this.categoryId}) : super(key: key);
  @override
  ShopCategoryDetailsScreenState createState() =>
      ShopCategoryDetailsScreenState();
}

class ShopCategoryDetailsScreenState
    extends BaseState<ShopCategoryDetailsScreen> {
//  List<String> listItems = ["one","two","three","four","five","two","three","four","one","two","three","four","one","two","three","four"];
  ...
  void initState() {
    ...
    // getTheFilteredData();
    listItems = storeController.itemsList.value;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        body: Obx(() => getRootContainer().withProgressIndicator(
  ...
  Widget getRootContainer() {
    return GridView.count(
      mainAxisSpacing: 8,
      crossAxisSpacing: 8,
      childAspectRatio: Get.width * 0.5 / 276,
      padding: EdgeInsets.all(16),
      crossAxisCount: 2,
      children: listItems.map((e) => ProductItem(e)).toList(),
    );
  }
  ...
  Widget dealsTileFirstRow(Item categoryItem) {
    return Column(
      children: [
        ...
        SizedBox(height: 12),
        Text(categoryItem.itemName, style: TextStyles.body2222)
      ],
    );
  }
  ...
  Widget dealsTileSecondRow(Item categoryItem) {
    return Column(
      children: [
        Text(categoryItem.category[0].name,
        ...
        SizedBox(height: 6),
        // Rp 5.300-6.600
        Text(categoryItem.price.toString(), style: BaseTextStyles.subtitle3222),
        SizedBox(height: 8),
        Container(
            height: 32,
  ...
  Widget titleWidget() {
    return Text(
      getTranslation(Strings.meatandseafood),
  ...
  Widget iconsWidget() {
    return Row(children: [searchIcon(), cartIcon()]);
  }
  ...
  void getTheFilteredData() {
    listItems = storeController.itemsList;
    //for now there is nothing like category Id in the catalogue response
    // .where((element) => element.category.id == widget.categoryId)
    // .toList();
  }
}
