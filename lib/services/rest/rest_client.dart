...
abstract class RestClient {
  ...
  Future<BaseResponse> validateOtp(
      @Body() AuthRequestWithData authRequestWithData);
  ...
  Future<BaseResponse> updateProfile(@Header("Authorization") String token,
      @Body() CustomerProfile updateProfileRequest);

  @GET(API.customer_profile)
  Future<CustomerProfile> getCustomerInfo(
      @Header("Authorization") String token, @Path() String customerId);

  //Order management API
  // createStore(Store store);
  @POST(API.create_store)
  Future<Store> createStore(
      @Header("Authorization") String token, @Body() Store store);

  @POST(API.create_owner)
  Future<Owner> createOwner(
      @Header("Authorization") String token, @Body() Owner owner);
  ...
  @GET(API.create_store)
  Future<List<StoreTypeModel>> getStoreTypes(
...
// Orders api's
  ...
  Future<Order> createOrder(
      @Header("Authorization") String token, @Body() Order order);

  // @POST(API.create_order)
  // Future<Order> deleteOrder(@Header("Authorization") String token,@Body()  String orderId);

  @PUT(API.update_order)
  Future<Order> updateOrder(@Header("Authorization") String token,
      @Body() Order order, @Path() String orderId);

  @GET(API.get_order)
  Future<Order> getOrderByOrderId(
      @Header("Authorization") String token, @Path() String orderId);
...
//  Future<List<Order>> getOrdersByMerchantId(@Header("Authorization") String token, @Path() String merchantId);
  ...
  @GET(API.create_order)
  Future<Catalogue> saveCatalogue(
      @Header("Authorization") String token, @Body() Catalogue catalogue);

  @GET(API.create_order)
  Future<Catalogue> updateCatalogue(
      @Header("Authorization") String token, @Body() Catalogue catalogue);

  @GET(API.get_toaddress)
  Future<ToAddressResponse> getToAddress(@Header("Authorization") String token,
      @Query("mobile_number") String mobile);

  @GET(API.customer_profile_by_firebase_id)
  Future<CustomerProfile> getCustomerInfoByFirebaseId(
      @Header("Authorization") String token,
