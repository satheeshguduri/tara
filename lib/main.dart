...
class TestWidget extends StatelessWidget {
          ...
          child: Column(
            ...
            children: [
              OutlineButton.icon(
                  ...
                  icon: Image.asset(Assets.ic_chat, width: 24, height: 24),
                  label: Text("Get MyAccounts")),
              OutlineButton.icon(
                  onPressed: () async {
                    await getBeneficiaries();
                  },
                  icon: Image.asset(Assets.ic_chat, width: 24, height: 24),
                  label: Text("Get Beneficiaries")),
              FlatButton(
                  onPressed: () async {
                    print("initiating the session");

                    // var commonRegistrationRequest = CommonRegistrationRequest(acquiringSource: AcquiringSourceBean());
                    var commonRegistrationRequest = await BaseRequestHelper()
                        .getCommonRegistrationRequest(
                            mobileNumber: "+919542829992");
                    await getIt
              ...
              RaisedButton(
                  onPressed: () async {
                    var commonRegistrationRequest = CommonRegistrationRequest(
                        acquiringSource: AcquiringSourceBean());
                    await getIt
                        .get<DeviceRegisterRepository>()
                        .initiateSession(commonRegistrationRequest);
              ...
              OutlineButton.icon(
                  onPressed: () async {
                    //  On contact Hit  ==> get the benId from the response
                    await getBeneficiaryDetails();
                  },
                  icon: Image.asset(Assets.ic_chat, width: 24, height: 24),
                  label: Text("Get Beneficiary Details")),
              OutlineButton.icon(
                  onPressed: () async {
                  ...
                  icon: Image.asset(Assets.ic_chat, width: 24, height: 24),
                  label: Text("Initiate Transaction")),
              ...
              OutlineButton.icon(
                  ...
                  icon: Image.asset(Assets.ic_chat, width: 24, height: 24),
                  label: Text("Validate Mobile")),
              OutlineButton.icon(
                  onPressed: () async {
                    //  On contact Hit  ==> get the benId from the response
                    await addBeneficiary(
                        mobile: "9295909790",
                        accountNo: "333333333333333",
                        bic: "CENAID00001",
                        name: "satheesh");
              ...
              OutlineButton.icon(
                  onPressed: () async {
                    ...
                    await HelpCrunch.initialize(
                      ...
                      appId: 1,
                      appSecret:
                          'oUhoF0PF0GASUWAY6O+L7luNkTeJRUkLNXUJmrYJ5N6Zr62xx7TEv81SvVcQCkv8VcU5kAmJ8kwNOVIyEuRvvQ==',
                    );
              ...
              OutlineButton.icon(
                  onPressed: () async {
                    //  On contact Hit  ==> get the benId from the response
                    await payViaCreditCard("35f6363c72d7410bbcf62efd19cdde9d",
                        100, "test payment", "498765******8769");
                    // await addBeneficiary(mobile:"9865327410",accountNo: "9865327410",bic: "CENAID00001",name:"bene by account");
                  },
                  icon: Image.asset(Assets.ic_chat, width: 24, height: 24),
  ...
  payViaCreditCardCardNew(num transactionAmount, String description) async {
    var request = CreateCardOrPayRequest(
      is_transaction: true,
      transaction:
          TransactionBean(amount: transactionAmount, description: description),
    );
  ...
  payViaCreditCard(String mcPaymentCardId, num transactionAmount,
    ...
    if (response.isRight()) {
      ...
      if (finalResponse?.data?.seamless_url?.isNotEmpty ?? false) {
        Get.to(CommonWebViewScreen(
            title: "Paying with Registered Credit card",
            type: WebViewType.PAYMENT,
            url: finalResponse.data.seamless_url));
      }
    }
  ...
  getBeneficiaries() async {
    ...
    if (isSessionInitiated) {
      ...
      var response =
          getIt.get<TransactionRepository>().getBeneficiaries(queries);
      print(response);
    }
  }
  ...
  Future validateMobile(
    ...
    if (isSessionInitiated) {
      ...
      var response =
          getIt.get<TransactionRepository>().validateMobile(validationRequest);
    }
  }
  ...
  Future addBeneficiary(
      ...
      String bic,
      String name,
      String accountType = "SAVINGS"}) async {
    var isSessionInitiated =
        await getIt.get<DeviceRegisterRepository>().checkAndInitiateSession();
    ...
    if (isSessionInitiated) {
      ...
      if (accInitResp.isRight()) {
        ...
        if (accInitiateResponse.success) {
          var deviceInfo = await BaseRequestHelper()
              .getDeviceInfoBeanWithPSP(mobileNumber: "9542829992");
  ...
  Future validateOtpAndTrackTransaction(
    ...
    var encValid = await CryptoHelper().encryptBankData(
        "$txnId|${fetchOTPResponse.otpChallengeCode}|${Random.secure()}",
        d.bankKi,
        d.publicKey);
  ...
  Future validateOtpAndTrack(String txnId, FetchOtpResponse fetchOTPResponse,
      RetrieveKeyResponse d, var deviceInfo, String bic) async {
    var encValid = await CryptoHelper().encryptBankData(
        "$txnId|${fetchOTPResponse.otpChallengeCode}|${Random.secure()}",
        d.bankKi,
        d.publicKey);
  ...
  Future registerDevice() async {
    ...
    if (isSessionInitiated) {
      ...
      if (data.isRight()) {
        ...
        var splIdentifier =
            await getIt.get<SessionLocalDataStore>().getIdentifier();
        ...
        var userRegResponseEither = await getIt
            .get<DeviceRegisterRepository>()
        ...
        if (userRegResponseEither.isRight()) {
          ...
          print(
              "======================== Register user Transaction Response========================================");
          ...
          await getIt
              .get<DeviceRegisterRepository>()
              .trackRegistration(trackRequest);
        }
      }
    } else {}
  }
  ...
  payNow() async {
    ...
    if (isSessionInitiated) {
      var toMobileNumber =
          "8368951368"; //"8106170677";//"9295909790";  should be without country code
      var amount = "100";
      ...
      var bic =
          "CENAID00001"; // get these from Search beneficiary call by passing the mobile number to the api
      var cvvValue = "123";

      var customerProfile = await getCustomerProfile2();

      var initiatorAccountId =
          customerProfile.mappedBankAccounts[0].accountTokenId;
        ...
        if (preTransactionResponse.success) {
          ...
          if (tResp.isRight()) {
            ...
            if (initiateTransactionResponse.success) {
              ...
              var respo = await getIt.get<TransactionRepository>().retrieveKey(
                  retrieveKeyRequest, TransactionType.FINANCIAL_TXN, txnId);
              if (respo.isRight()) {
                var retriveKeyResponse = respo.getOrElse(() => null);
                print(jsonEncode(retriveKeyResponse.toJson()));
                var cvv = await CryptoHelper().encryptBankData(
                ...
                var authorizeReq = AuthorizeRequest(
                    ...
                    authorizePINCred:
                        AuthorizePINCredBean(credType: "MPIN", credValue: cvv));
                var respo2 = await getIt.get<TransactionRepository>().authorize(
                    authorizeReq,
                  ...
                  if (res.commonResponse.success) {
                    ...
                    Timer(Duration(seconds: 2), () async {
                      ...
                      if (fetchOTPR.isRight()) {
                        ...
                        var re = await validateOtpAndTrackTransaction(
                            txnId,
                            fetchOTPResponse,
                            retriveKeyResponse,
                            deviceInfo,
