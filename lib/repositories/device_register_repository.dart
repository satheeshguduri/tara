...
abstract class DeviceRegisterRepository {
  Future<Either<Failure, TokenResponse>> initiateSession(
      CommonRegistrationRequest commonRegistrationRequest);
  Future<Either<Failure, TokenResponse>> getAppToken(
      CommonRegistrationRequest commonRegistrationRequest);
  Future<Either<Failure, TokenResponse>> getAppTokenMerchant(
      CommonRegistrationRequest commonRegistrationRequest);
  Future<Either<Failure, TokenResponse>> getPrivateAccessToken(
      CommonRegistrationRequest commonRegistrationRequest);
  Future<Either<Failure, RegisterResponse>> register(
      RegisterRequest registerRequest);
  Future<Either<Failure, void>> registerUserTxn(
      UserRegistrationTxnRequest registerRequest);
  Future<Either<Failure, UserRegistrationResponse>> registerUser(
      UserRegistrationRequest userRegistrationRequest);
  Future<Either<Failure, TrackRegistrationResponse>> trackRegistration(
      CommonRegistrationRequest commonRegistrationRequest);
...
class DeviceRegisterRepositoryImpl implements DeviceRegisterRepository {
  ...
  DeviceRegisterRepositoryImpl(this.sessionLocalDataStore, this.networkInfo,
      this.pspRemoteDataSource, this.umpsRemoteDataSource);
  ...
  Future<Either<Failure, TokenResponse>> getAppToken(
    ...
    try {
      var encryptedBody = await CryptoHelper()
          .encrypt(json.encode(commonRegistrationRequest.toJson()));
      print("Encrypted:=>" + encryptedBody);
      var response = await pspRemoteDataSource.getAppToken(
          PSPConfig.MERCHANT_KI, encryptedBody);
  ...
  Future<Either<Failure, TokenResponse>> getAppTokenMerchant(
    ...
    print("For Merchant App Token==> " +
        jsonEncode(commonRegistrationRequest.toJson()));
    try {
      var encryptedBody = await CryptoHelper()
          .encrypt(json.encode(commonRegistrationRequest.toJson()));
      var response = await pspRemoteDataSource.getAppTokenMerchant(
          PSPConfig.MERCHANT_KI, encryptedBody);
      var decryptedResponse = await CryptoHelper().decrypt(response);
      var map = getMap(decryptedResponse);
      if (map?.isNotEmpty ?? false) {
        ...
        if (tokenResponse?.token?.isNotEmpty ?? false) {
          // await sessionLocalDataStore.setToken(tokenResponse);
          print("Merchant App Token response ::" +
  ...
  Future<Either<Failure, TokenResponse>> getPrivateAccessToken(
    ...
    try {
      var response = await pspRemoteDataSource
          .getPrivateAccessToken(commonRegistrationRequest);
  ...
  @override
  Future<Either<Failure, void>> registerUserTxn(
      UserRegistrationTxnRequest userRegistrationTxnRequest) async {
    try {
    ...
    try {
      ...
      if (response?.commonResponse?.symmetricKey?.isNotEmpty ?? false) {
        var decryptedBody = await CryptoHelper().decryptDataWithSymmetricKey(
            response.userRegistrationResponsePayloadEnc,
            response?.commonResponse?.symmetricKey,
            sessionInfo.sessionKey,
            true);
        var finalResponse =
            UserRegistrationResponse.fromJson(getMap(decryptedBody));
  ...
  Future<Either<Failure, TrackRegistrationResponse>> trackRegistration(
    ...
    try {
      var response = await pspRemoteDataSource
          .trackRegistration(commonRegistrationRequest);
      return Right(response);
    } catch (e) {
      return Left(Failure.fromServerError(e));
    }
